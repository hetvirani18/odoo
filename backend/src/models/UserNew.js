...
const userSchema = new mongoose.Schema(
  {
    ...
    isVerified: {
      type: Boolean,
      default: true, // Start with verified users for simplicity
    },
    avatar: {
      type: String,
      default: '',
    ...
    // Simplified refresh token storage
    refreshTokens: [
      {
        token: String,
        expiresAt: Date,
...
userSchema.methods.generateToken = function () {
  return jwt.sign(
    ...
    {
      expiresIn: process.env.JWT_EXPIRY || '24h', // Longer expiry for simplicity
    }
  );
};
